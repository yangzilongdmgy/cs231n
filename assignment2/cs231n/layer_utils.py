...
def affine_relu_dropout_forward(x, w, b, dropout_param):
    ...
    drop_out, drop_cache = dropout_forward(relu_out, dropout_param)
    cache = (fc_cache, relu_cache, drop_cache)
    return drop_out, cache
...
def affine_relu_batchnormalization_forward(x, w, b, gamma, beta, bn_param):
    """Convenience layer that performs an affine transform followed by a ReLU.
...
def affine_relu_batchnormalization_backward(dout, cache):
    """Backward pass for the affine-relu convenience layer.
    """
